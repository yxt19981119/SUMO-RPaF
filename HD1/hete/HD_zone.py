...
df_car = pd.read_excel('./sumo_config/vehicle_info_hete.xlsx', sheet_name='information table')  # 替换为你的文件名和工作表名称
...
RPV2Zone = {'1':'1','2':'1','3':'1','4':'1','5':'2','6':'2','7':'3','8':'3','9':'4','10':'4'}
Zone2RPV = {'1':['1','2','3','4'],'2':['5','6'],'3':['7','8'],'4':['9','10']}
    ...
    if len(unmatched_request) == 0 or exit_flag == True:  # 没人或者车满的情况
        ...
        for index,each in enumerate(Zone2RPV[zone_id]):
            ...
            if (c[each]) < 5:  # 由于乘客进入路网之后，信息都为已知确定
                ...
                dis[0][index] = temp_distance - car_pos - (street_space - person_pos)  # 这个100后面要写的更加generalize
            elif (c[each]) >= 5:
                dis[0][index] = 1e10
        if min(dis[0]) <= Delta:  # 如果该乘客距离所有available车辆的位置均超过最大允许，那么该乘客将失去资格
            temp = int(np.argmin(dis[0]))  # 获取最近车辆id
