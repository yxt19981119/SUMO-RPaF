...
df_car = pd.read_excel('./sumo_config/vehicle_info_homo.xlsx', sheet_name='N27')  # option [fleet size]
...
def distribute(Delta,unmatched_request,step,zone_request):
    ...
    if len(unmatched_request) == 0 or exit_flag == True:  # no request occur
        print('nothing occur')
    elif len(unmatched_request) == 1:  # one request occur
        print('one request occur')
        ...
        if min(dis[0]) <= Delta:  #
            ...
            p[unmatched_request[0]] = 2  #
            print('exceed buffer distance',unmatched_request[0])
    elif len(unmatched_request) > 1:
        print('more than one request')
        for c_key in c.keys():
            if c[c_key] >= car_tau[c_key]+1:
                ...
                empty_seat = car_tau[c_key]+1-c[c_key]
...
def run():
    ...
    while traci.simulation.getTime() < end_time:
        ...
        for each1 in list(traci.person.getIDList()):
            if each1 not in traci.edge.getLastStepPersonIDs(edgeID=terminal_reverse) and passenger_hashmap[each1] == 1:
                if math.floor(step-passenger_hash_map[each1][0]) >= tolerance_time:
                    ...
                    zone_cancel_num[passenger_hash_map[each1][2]] = zone_cancel_num[
                                                                        passenger_hash_map[each1][2]] + 1
                    count1 = count1 + 1
        print(f"number of cancel request: {count1}")
        print(f"zone cancel request: {zone_cancel_num}")
        ...
        print(f"car tau {car_tau}")
        print(f"vehicle dispatch time {vehicle_time}")
        # dispatch_algorithm
        for key in Car_hash_map.keys():
            if car_relocate[key] == 1:
                if traci.vehicle.getRoadID(vehID=key) == car_hash_map[key][0]:
                    if car_hash_map[key][1] == '201':
                        car_hash_map[key] = car_hash_map[key][0:1]
                    elif car_hash_map[key][1] != '201':
                        car_hash_map[key] = car_hash_map[key][0:2]
                    car_relocate[key] = -1
                    car_tau[key] = 4
                    vehicle_time[key] = step+tolerance_time
                ...
                if car_flag[key] == -1:  # outbound
                    if 2 <= len(Car_hash_map[key]) <= 5 and passenger_hashmap[car_hash_map[key][1]] == 2:
                        vehicle_time[key] = passenger_hash_map[car_hash_map[key][1]][0] + tolerance_time
                    elif 2 <= len(Car_hash_map[key]) <= 5 and passenger_hashmap[car_hash_map[key][1]] == 4:
                        vehicle_time[key] = vehicle_time[key]
                    if len(Car_hash_map[key]) < car_tau[key]+1 and len(Car_hash_map[key]) <= 5:
                        ...
                        for i in range(6 - len(Car_hash_map[key])):
                            Car_hash_map[key].append(str(i + 201))  #
                        vehicle_pick[key].append(step)
                    if step >= vehicle_time[key] and 2 <= len(Car_hash_map[key]) <= 5:  # and 2 <= len(Car_hash_map[key]) <= 5
                        print('taxi%s meet the dispatch' % key)
                        route, routeEdgeid, routePosid = addRoute(Car_hash_map[key], key)
                        print(route)
                        print(routeEdgeid)
                        print(routePosid)
                    ...
                    if key in hub_edge:
                        if traci.vehicle.getLanePosition(vehID=key) < 4885 and traci.vehicle.getLanePosition(vehID=key) > 4882:
                            ...
                            if car_flag[key] == -1:
                                car_hash_map[key] = [car_hash_map[key][0],'202','203','204','205']
                                car_hashmap[key] = 5
                                car_flag[key] = 1
                        ...
                        if traci.vehicle.getLanePosition(vehID=key) > 501 and Car_hashmap[key] == -1:
                            ...
                            if len(inbound_demand) != 0:
                                ...
                                if len(unmatched_request) == 0:
                                    ...
                                    car_stop[key] = 20
                                    car_hash_map[key] = [net.getEdge(temp_edge).getID(), '201', '202', '203', '204']
                                else:
                                    ...
                                    if urgent_request == []:
                                        ...
                                        passenger_hashmap[urgent_request] = 4
                                        ...
                                        car_hash_map[key] = [net.getEdge(temp_edge).getID(),urgent_request,'202','203','204']
                                car_relocate[key] = 1
                                Car_hashmap[key] = 1
        ...
        print(f"pick-up list {pick_up_list}")
        step+=1
    traci.close()
    sys.exit()
